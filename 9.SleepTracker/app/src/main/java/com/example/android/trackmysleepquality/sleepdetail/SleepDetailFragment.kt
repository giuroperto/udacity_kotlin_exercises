...
//        To use the ViewModel with data binding, you have to explicitly give the binding object
//        a reference to it
        binding.sleepDetailViewModel = sleepDetailViewModel

        binding.lifecycleOwner = this

//        Add an Observer to the state variable for Navigating when a Quality icon
//        is tapped.
        sleepDetailViewModel.navigateToSleepTracker.observe(viewLifecycleOwner, Observer {
            if (it === true) { // Observed state is true
                this.findNavController().navigate(
                    SleepDetailFragmentDirections.actionSleepDetailFragmentToSleepTrackerFragment())
//                Read state to make sure we only navigate once, even if the device
